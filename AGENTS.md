...
Never commit secrets beyond the published Sentry DSN. Switch environments via `.env` (set `EXPO_PUBLIC_API_ENV=DEV|STAGING|PROD` and optional `EXPO_PUBLIC_API_BASE_URL`) instead of editing source; avoid hard-coding local hosts in other files. After changing native settings or config, rerun the relevant Expo command (`npm run ios` / `npm run android`) so updates propagate.
